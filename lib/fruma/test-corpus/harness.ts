 ...
 * Honours Mapping-agent confirmed overlays when present.
 */
export function runCorpusHarness(args?: {
  headerOverlays?: Record<string, StandardField>;
}): CorpusHarnessResult {
  const overlays = args?.headerOverlays ?? confirmedHeaderOverlays(TEST_SURFACE);
  const rows = TEST_FACTORIES.map((factory) => depositFactory(factory, overlays));
